/*****************************************************************************
 *   kmtricks
 *   Authors: T. Lemane
 *
 *  This program is free software: you can redistribute it and/or modify
 *  it under the terms of the GNU Affero General Public License as
 *  published by the Free Software Foundation, either version 3 of the
 *  License, or (at your option) any later version.
 *
 *  This program is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *  GNU Affero General Public License for more details.
 *
 *  You should have received a copy of the GNU Affero General Public License
 *  along with this program.  If not, see <http://www.gnu.org/licenses/>.
 *****************************************************************************/

#pragma once
#include <string>
#include <exception>
#include <fstream>
#include <cstring>
#include <emmintrin.h>
#include <iostream>
#include <cassert>
#include <iomanip>

using namespace std;

namespace km
{

const uint8_t reverseb[] = {
  ,
};

void __sse_trans(uint8_t const *inp, uint8_t *out, int nrows, int ncols);

class BitMatrix
{
public:

  BitMatrix(size_t n, size_t m, bool lendian, bool def = false)
    : _n(n / 8), _m(m), _nb(n), _mb(m * 8), _le(lendian)
  {
    check8();
    matrix = new uint8_t[_nb * _m]();
    if ( def )
      memset(matrix, 0xFF, _nb * _m);
  }

  BitMatrix(uint8_t *mat, size_t n, size_t m, bool lendian)
    : _n(n / 8),
      _m(m),
      _nb(n),
      _mb(m * 8),
      _le(lendian)
  {
    check8();
    matrix = mat;
  }

  ~BitMatrix()
  {
    if ( matrix )
      delete[] matrix;
  }

  void set_bit(size_t i, size_t j, bool value)
  {
    size_t offset = (i * _mb + j) / 8;
    size_t pos = (i * _mb + j) % 8;
    uint8_t mask = _le ? 0x1 << pos : 0x80 >> pos;
    if ( value )
      matrix[offset] |= mask;
    else
      matrix[offset] &= ~(mask);
  }

  void tog_bit(size_t i, size_t j)
  {
    size_t offset = (i * _mb + j) / 8;
    size_t pos = (i * _mb + j) % 8;
    uint8_t mask = _le ? 0x1 << pos : 0x80 >> pos;
    matrix[offset] ^= mask;
  }

  bool get_bit(size_t i, size_t j)
  {
    size_t offset = (i * _mb + j) / 8;
    size_t pos = (i * _mb + j) % 8;
    uint8_t mask = _le ? 0x1 << pos : 0x80 >> pos;
    return matrix[offset] & mask;
  }

  void set_byte(size_t i, size_t j, uint8_t value)
  {
    matrix[i * _m + j] = _le ? value : reverseb[value];
  }

  void tog_byte(size_t i, size_t j)
  {
    size_t offset = (i * _m + j);
    matrix[offset] = matrix[offset] ^ 0xFF;
  }

  uint8_t get_byte(size_t i, size_t j)
  {
    return matrix[i * _m + j];
  }

  uint8_t *get_line(size_t i)
  {
    uint8_t *line = new uint8_t[_n];
    memcpy(line, &matrix[i * _m], _n);
    return line;
  }

  size_t get_size_in_byte()
  {
    return _nb * _m;
  }

  size_t get_nb_lines()
  {
    return _nb;
  }

  size_t get_nb_cols()
  {
    return _mb;
  }

  void clear()
  {
    memset(matrix, 0, _nb * _m);
  }

  void print_bytes()
  {
    std::cout << "\n\n";
    for ( size_t i = 0; i < _nb; i++ )
    {
      for ( size_t j = 0; j < _m; j++ )
        std::cout << "0x" << std::setfill('0') << std::setw(2) << std::hex << static_cast<int>(matrix[i * _m + j]) << " ";
      std::cout << "\n";
    }
    std::cout << std::endl;
  }

  void print_bits()
  {
    std::cout << "\n\n";
    string v;
    uint8_t tmp;
    for ( size_t i = 0; i < _nb; i++ )
    {
      for ( size_t j = 0; j < _m; j++ )
      {
        tmp = matrix[i * _m + j];
        for ( int p = 7; p >= 0; p-- )
        {
          v = ((tmp >> p) & 0x1) ? "1" : "0";
          std::cout << v;
        }
        std::cout << " ";
      }
      std::cout << "\n";
    }
    std::cout << std::endl;
  }

  BitMatrix *transpose()
  {
    uint8_t *mt = new uint8_t[_nb * _m];
    __sse_trans(matrix, mt, _nb, _mb);
    return new BitMatrix(mt, _mb, _n, !_le);
  }

public:
  uint8_t *matrix;

private:
  inline void check8()
  {
    if ( _nb % 8 != 0 )
      throw runtime_error("n % 8 != 0 -> n=" + to_string(_nb));
    else if ( _mb % 8 != 0 )
      throw runtime_error("m % 8 != 0 -> m=" + to_string(_mb));
  }

private:
  string _fpath;
  size_t _n;  // in bytes
  size_t _m;  // in bytes
  size_t _nb; // in bits
  size_t _mb; // in bits
  bool _le; // true if little endian
};

// from https://mischasan.wordpress.com/2011/10/03/the-full-sse2-bit-matrix-transpose-routine/
inline void __sse_trans(uint8_t const *inp, uint8_t *out, int nrows, int ncols)
{
#   define INP(x, y) inp[(x)*ncols/8 + (y)/8]
#   define OUT(x, y) out[(y)*nrows/8 + (x)/8]
  ssize_t rr, cc, i, h;
  union
  {
    __m128i x;
    uint8_t b[16];
  } tmp;
  assert(nrows % 8 == 0 && ncols % 8 == 0);

  // Do the main body in 16x8 blocks:
  for ( rr = 0; rr <= nrows - 16; rr += 16 )
  {
    for ( cc = 0; cc < ncols; cc += 8 )
    {
      for ( i = 0; i < 16; ++i )
        tmp.b[i] = INP(rr + i, cc);
      for ( i = 8; --i >= 0; tmp.x = _mm_slli_epi64(tmp.x, 1))
        *(uint16_t *) &OUT(rr, cc + i) = _mm_movemask_epi8(tmp.x);
    }
  }

  if ( nrows % 16 == 0 )
    return;
  rr = nrows - nrows % 16;

  // The remainder is a block of 8x(16n+8) bits (n may be 0).
  //  Do a PAIR of 8x8 blocks in each step:
  for ( cc = 0; cc <= ncols - 16; cc += 16 )
  {
    for ( i = 0; i < 8; ++i )
    {
      tmp.b[i] = h = *(uint16_t const *) &INP(rr + i, cc);
      tmp.b[i + 8] = h >> 8;
    }
    for ( i = 8; --i >= 0; tmp.x = _mm_slli_epi64(tmp.x, 1))
    {
      OUT(rr, cc + i) = h = _mm_movemask_epi8(tmp.x);
      OUT(rr, cc + i + 8) = h >> 8;
    }
  }
  if ( cc == ncols )
    return;

  //  Do the remaining 8x8 block:
  for ( i = 0; i < 8; ++i )
    tmp.b[i] = INP(rr + i, cc);
  for ( i = 8; --i >= 0; tmp.x = _mm_slli_epi64(tmp.x, 1))
    OUT(rr, cc + i) = _mm_movemask_epi8(tmp.x);
}

}; // end of namespace km